...
export default function HomePage() {
  ...
  const handleShare = async () => {
    try {
      ...
      // Extract filename from imageUrl (og-{fid}-{timestamp}.png)
      const imageFilename = imageUrl.split('/').pop().replace('.png', '');

      // Create Warpcast share URL with image and app URL as embeds
      const encodedText = encodeURIComponent(shareText);
      const encodedAppUrl = encodeURIComponent(`${window.location.origin}?image=${imageFilename}`);
